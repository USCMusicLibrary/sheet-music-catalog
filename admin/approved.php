...
if (isSuper()){
  $statement = $mysqli->prepare("SELECT id, mid,title,call_number,series,larger_work,collection_source,donor,scanning_technician,media_cataloguer_id,reviewer_id,status FROM records WHERE status='approved' ORDER BY date_modified DESC");
}
else {
  $statement = $mysqli->prepare("SELECT id, mid,title,call_number,series,larger_work,collection_source,donor,scanning_technician,media_cataloguer_id,reviewer_id,status FROM records WHERE status='approved' AND media_cataloguer_id=? ORDER BY date_modified DESC");
