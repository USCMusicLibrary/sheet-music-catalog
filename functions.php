...
function insertDocDb($doc,$status){
  ...
  $statement = $mysqli->prepare("INSERT INTO records (id,mid,title,call_number,series,larger_work,collection_source,donor,start_year,end_year,scanning_technician,media_cataloguer_id,reviewer_id,admin_notes,status,date_created,date_modified)"
                  ." VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())");
  //var_dump($doc);
  $statement->bind_param("iisssssssssiiss",$id, $mid, 
              $title, 
              $call_number, 
              $series, 
              $larger_work, 
